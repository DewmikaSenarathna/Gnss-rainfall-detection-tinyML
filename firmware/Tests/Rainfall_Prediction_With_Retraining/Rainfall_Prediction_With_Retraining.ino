...
void connectWiFi()
{
  ...
  sendCommand("AT+CWJAP=\"YOUR_WIFI\",\"PASSWORD\"",6000);

}

void sendToThingSpeak(float temp,float hum,int snr,int rain,int pred)
{

  String data = "GET /update?api_key="+apiKey;
  data += "&field1="+String(temp);
  data += "&field2="+String(hum);
  data += "&field3="+String(snr);
  data += "&field4="+String(rain);
  data += "&field5="+String(pred);

  sendCommand("AT+CIPSTART=\"TCP\",\"api.thingspeak.com\",80",2000);

  sendCommand("AT+CIPSEND="+String(data.length()+4),2000);
