...
void loop()
{
  ...
  features[0] = rain_value;
  features[1] = humidity;
  features[2] = sats[0].azimuth;
  features[3] = temperature;
  features[4] = max_snr;
  features[5] = sats[0].elevation;
  features[6] = sats[0].prn;

  score(features, result);

  int prediction = 0;

  if(result[1] > result[prediction])
      prediction = 1;

  if(result[2] > result[prediction])
      prediction = 2;
...
void show_display(int prediction)
{
  ...
  if(prediction==0)
    display.print("RAIN:SUNNY");

  if(prediction==1)
    display.print("RAIN:LOW");

  if(prediction==2)
    display.print("RAIN:HEAVY");
